(function (a, b) {
  ...
  a.MixItUp.prototype = {
    ...
    _init: function (f, d) {
      ...
      if (c.controls.toggleFilterButtons) {
        ...
        for (var e = 0; e < c._toggleArray.length; e++) {
          c._updateControls(
            { filter: c._toggleArray[e], sort: c._activeSort },
            true
          );
        }
      } else {
        if (c.controls.enable) {
          c._updateControls({ filter: c._activeFilter, sort: c._activeSort });
        }
      }
      c._filter();
      c._init = true;
        ...
        g = function (l) {
          for (var k = 0; k < j.length; k++) {
            if (j[k] + "Transition" in l.style) {
              return { prefix: "-" + j[k].toLowerCase() + "-", vendor: j[k] };
            }
          }
    ...
    _parseSort: function (g) {
      ...
      for (var e = 0; e < j.length; e++) {
        var h = typeof g === "string" ? j[e].split(":") : ["custom", j[e]],
          f = { sortBy: d._helpers._camelCase(h[0]), order: h[1] || "asc" };
        c.push(f);
    ...
    _parseMultiMixArgs: function (f) {
      var e = this,
        d = { command: null, animate: e.animation.enable, callback: null };
    ...
    filter: function () {
      ...
      c.multiMix({ filter: d.command }, d.animate, d.callback);
    },
    sort: function () {
      var c = this,
        d = c._parseMultiMixArgs(arguments);
      c.multiMix({ sort: d.command }, d.animate, d.callback);
    },
    changeLayout: function () {
      var c = this,
        d = c._parseMultiMixArgs(arguments);
      c.multiMix({ changeLayout: d.command }, d.animate, d.callback);
    },
    multiMix: function () {
      var c = this,
        d = c._parseMultiMixArgs(arguments);
